...
using namespace std;
/**
 * @brief Constructor para la pantalla del tercer nivel
...
void Level3::setValues(int p, int v, int n){
    ...
    connect(movementPacmanMobile, &QTimer::timeout, this, &Level3::startSocketServer);
    movementPacmanMobile -> setInterval(500);
    movementPacmanMobile -> start();

    exeMovementPacmanMobile = new QTimer(this);
    connect(exeMovementPacmanMobile, &QTimer::timeout, this, &Level3::MoveMobile);
...
void Level3::startSocketServer(){
    SocketThread3* socketThread = new SocketThread3(this);
    connect(socketThread, &SocketThread3::finished, socketThread, &SocketThread3::deleteLater);
    socketThread -> start();
}

void Level3::SocketServer() {
...
void Level3::PlacePowerRandomPos(){
    isTherePower = true;
...
void Level3::comerPoderes(){
    if(isTherePower){
        ...
        if(pacmanX == powerX && pacmanY == powerY){
            ...
            QTimer::singleShot(5000, this, &Level3::pararEjecucion);
            //revisarChoque -> stop();
...
void Level3::pararEjecucion(){
    //revisarChoque -> setInterval(500);
    //revisarChoque -> start();
    isPowerActivated = false;
}

SimpleList<SimpleList<int>> Level3::PathfindingA(int beginX, int beginY, int endX, int endY){
...
SimpleList<SimpleList<int>> Level3::FindPath(SimpleList<SimpleList<int>> completeCasillas, SimpleList<SimpleList<int>> hCasillas, int beginX, int beginY, int endX, int endY){
